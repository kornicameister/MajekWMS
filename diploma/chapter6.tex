...
\section{Diagramy, zrzuty ekranowe oraz funkcje}
	\subsection{Logowanie do aplikacji przy pierwszym uruchomieniu}
		...
		wystosowywane jest okno, takie jak na rysunku \label{c7:fig:app::login}. Mechanizm został zaimplementowany
		po stronie klienta, dzięki użytecznym i gotowym obiektom klas, które dają możliwość utrzymania 
		wiedzy o aktualnie zalogowanym użytkowniku pomiędzy kolejnymi odświeżeniami witryny. \\
		Samo logowanie został oparte o prosty, ale rozszerzalny mechanizm weryfikacja nazwy użytkownika
		oraz jego hasła. Można to łatwo zmienić, ponieważ operacja autoryzacja została wyodrębniona
		do samodzielnej \textbf{klasy servlet}. Korzyścią tego, a zaraz wytłumaczeniem prostoty rozszerzenia
		...
		\begin{listing}[H]
			\inputminted[
				...
				lastline=62,
	            numbersep=10pt
			]{java}{../application/javaSrc/wms/servlet/WMSAuthAgent.java}	
